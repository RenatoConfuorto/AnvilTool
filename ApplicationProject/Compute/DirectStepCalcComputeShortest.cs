    ...
    public class DirectStepCalcComputeShortest : ComputeShortesBase
    {
        public override List<Move> Compute(int startingPos, int targetPos, List<Move> lastMoves)
        {
            ...
            while(iter < Cnst.MAX_ITER)
            {
                ...
                while (currentSeq.Count <  Cnst.MAX_SEQ_LENGHT)
                {
                    ...
                    if(diff == 0)
                    {
                        ...
                        //if (currentSeq.Count - startSeq.Count < Cnst.EXIT_SEQ_LEN)
                    ...
                    if(newPos <= Cnst.MIN_POS || newPos >= Cnst.MAX_POS)
                    {
                        Log("-------------------------");
        ...
        private List<Move> GetStartingMoves(List<Move> originalStarting, int max, int min, int target)
        {
            ...
            foreach (Move predefinedMove in Cnst.Moves)
            {
                ...
                for (int i = 0; i < originalStarting.Count - 1; i++) // Start with subsets of size 2
                {
                    ...
                    for (int j = i; j < originalStarting.Count; j++)
                    {
                        ...
                        if (newCandidate.Count < bestComb.Count && Math.Abs(candidateSum - bestSum) <= Cnst.START_SEQ_TOL)
                        {
        ...
        private Move TakePositive(int diff)
        {
            ...
            //return Cnst.Moves.FirstOrDefault(m => m.Delta)
        }
        ...
        private Move TakeSmallestPositive() => Cnst.Moves
            .Where(m => m.Delta > 0)
            .OrderBy(m => m.Delta)
            .FirstOrDefault();

        private Move TakeSmallestNegative() => Cnst.Moves
            .Where(m => m.Delta < 0)
            .OrderByDescending(m => m.Delta)
            .FirstOrDefault();
        #endregion
